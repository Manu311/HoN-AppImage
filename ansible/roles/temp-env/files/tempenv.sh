...
function start() {
	...
	unionfs -o cow,allow_other tempenv/files=RW:tempenv/persist:/ tempenv/chroot
	#mount --rbind tempenv/dev tempenv/chroot/dev
	#mount --make-rslave tempenv/chroot/dev
	#mount --bind /dev/pts tempenv/chroot/dev/pts
	#mount -t proc /proc tempenv/chroot/proc
	
	mount --rbind /dev tempenv/chroot/dev
	mount --make-rslave tempenv/chroot/dev
	mount -t proc /proc tempenv/chroot/proc
	mount --rbind /sys tempenv/chroot/sys
	mount --make-rslave tempenv/chroot/sys
	mount --rbind /tmp tempenv/chroot/tmp

	set +e
	chroot tempenv/chroot
	umount -R tempenv/chroot
	set -e
}
